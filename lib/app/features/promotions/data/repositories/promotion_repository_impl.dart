...
class PromotionRepositoryImpl implements PromotionRepository {
  ...
  Future<void> deletePromotion(String promotionId) =>
      remoteDataSource.deletePromotion(promotionId);
  ...
  Stream<List<PromotionEntity>> getAllPromotions() {
    return remoteDataSource.getAllPromotions().map(
      (snapshot) =>
  ...
  Stream<List<PromotionEntity>> getActivePromotions() {
    return remoteDataSource.getActivePromotions().map(
      (snapshot) =>
          snapshot.docs.map((doc) => PromotionModel.fromSnapshot(doc)).toList(),
    );
  }
  ...
  @override
  Future<PromotionEntity?> validateCoupon(
    String couponCode,
    String userId,
  ) async {
    ...
    final hasRedeemed = await remoteDataSource
        .getUserRedemption(promotion.id, userId)
        .then((doc) => doc.exists);
