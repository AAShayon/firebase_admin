...
class AuthRemoteDataSourceImpl implements AuthRemoteDataSource {
  ...
  Future<UserModel> signInWithEmailAndPassword(String email, String password) async {
    try {
      ...
      return await _handleUserAuthResult(userCredential.user);
    } on FirebaseAuthException catch (e) {
      throw AuthFailure.fromCode(e.code);
    } catch (_) {
      throw const AuthFailure();
  ...
  Future<UserModel> _handleUserAuthResult(User? user) async {
    if (user == null) throw const AuthFailure(message: 'User not found');

    final role = await checkUserRole(user.uid);
    return UserModel(
      id: user.uid,
      email: user.email,
      displayName: user.displayName,
  ...
  @override
  Future<UserRole> checkUserRole(String userId) async {
    try {
      final adminDoc = await _firestore.collection('admins').doc(userId).get();
      if (adminDoc.exists) {
        return adminDoc.data()?['isSuperAdmin'] == true
            ? UserRole.superAdmin
            : UserRole.admin;
      }

      final subAdminDoc = await _firestore.collection('subAdmins').doc(userId).get();
      return subAdminDoc.exists ? UserRole.subAdmin : UserRole.user;
    } catch (e) {
      throw AuthFailure(message: 'Failed to check user role: ${e.toString()}');
    }
  }
